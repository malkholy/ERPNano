    ...
    <div>
      ...
      <div style={{ marginBottom:16 }}>
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          style={{ width:"320px", padding:"9px 14px", background:"rgba(0,0,0,.07)",
            border:"1px solid rgba(0,0,0,.1)", borderRadius:10, color:"#1e293b",
      ...
      <div style={{ background:"#ffffff", borderRadius:12, border:"1px solid #e2e8f0", overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", padding:"12px 20px",
          borderBottom:"1px solid #e2e8f0", background:"rgba(0,0,0,.03)" }}>
        ...
        {loading && <div style={{ padding:48, textAlign:"center", color:"#64748b", fontSize:13 }}>⏳ Loading users...</div>}
        {!loading && filtered.length === 0 && !error && <div style={{ padding:48, textAlign:"center", color:"#64748b", fontSize:13 }}>No users found</div>}

        {!loading && filtered.map((row, i) => (
          <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 1fr", padding:"13px 20px", borderBottom:"1px solid #f1f5f9" }}
            ...
            <div style={{ fontSize:13, color:"#64748b", display:"flex", alignItems:"center" }}>{row.Username}</div>
          </div>
        ))}
      </div>
